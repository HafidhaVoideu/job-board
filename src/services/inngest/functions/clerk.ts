...
import { insertUserNotificationSettings } from "@/features/user/db/userNotificationsSettings";
import {
  deleteOrganization,
  insertOrganization,
  updateOrganization,
} from "@/features/organization/db/organization";
...
function verifyWebhook({
  ...
  return new Webhook(env.CLERK_WEBHOOK_SECRET_KEY).verify(raw, headers);
}
...
export const clerkCreateUser = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    const userId = await step.run("create-user", async () => {
      console.log("this event has started");
